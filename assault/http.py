...
async def worker(name, queue, results):
    """A function to take unmake requests from a queue abd perform the work then add results to the results list."""
...
async def distribute_work(url, requests, concurrency, results):
    """Divide up the work into batrches and collect the final results"""
    queue = asyncio.Queue()
